...
****************************************************************************/
%macro vf_prepare_ts_abt_pmix(mpVfPboProjName=pbo_sales_v2,
							mpPmixSalAbt=dm_abt.pmix_sal_abt,
							mpPromoW1=dm_abt.promo_w1,
							mpPromoD=dm_abt.promo_d,
							mpPboSales=dm_abt.TS_pbo_sales,
	...
	*quit;
	...
	timeData.timeSeries result =r /
		...
		table={caslib="casuser",name="pmix_sales_rest", groupby={"PBO_LOCATION_ID","PRODUCT_ID","CHANNEL_CD"} ,
	...
	%if %sysfunc(exist(casuser.PRICE)) eq 0 %then %do;
	...
	proc casutil;
	  droptable casdata="&lmvOutTabNamePmixSalAbt._dlv" incaslib="casuser" quiet;
	run;
	...
	proc casutil;
	 promote casdata="&lmvOutTabNamePmixSalAbt._dlv" incaslib="casuser" outcaslib="dm_abt";
