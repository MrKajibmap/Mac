%macro vf_restore_sales_gc;
	...
	/*входные таблицы casuser.pmix_sales, pbo_sales, выходные: pbo_sales_rest, pmix_sales_rest*/
	...
	proc fedsql sessref=casauto;
		create table CASUSER.means_to_restore{options replace=true} as
			...
			left join casuser.PMIX_SALES t2
	...
	proc casutil;
		promote casdata='pmix_sales_rest' incaslib='casuser' outcaslib='casuser';
		save casdata="pmix_sales_rest" incaslib="casuser" outcaslib="casuser" replace compress;
	run;
	...
	proc fedsql sessref=casauto;
		create table CASUSER.means_to_restore_gc{options replace=true} as
			select 
				...
				from CASUSER.pbo_cl_per_int t1 
				left join casuser.PBO_SALES t2
					on t1.channel_cd=t2.channel_cd 
					and t1.pbo_location_id=t2.pbo_location_id
					and (t2.sales_dt between intnx('day',t1.start_int_dt,-&delta.) and intnx('day',t1.start_int_dt,-1)
					or t2.sales_dt between intnx('day',t1.end_int_dt,1) and intnx('day',t1.end_int_dt,&delta.))
	...
	/*данные уже будут протянуты по дням*/
	proc casutil;
		promote casdata='pbo_sales_rest' incaslib='casuser' outcaslib='casuser';
		save casdata="pbo_sales_rest" incaslib="casuser" outcaslib="casuser" replace compress;
	run;
