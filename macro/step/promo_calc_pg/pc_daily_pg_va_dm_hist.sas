...
******************************************************************
*  НАЗНАЧЕНИЕ:
*     Макрос для ежедневной загрузки данных в DM_REP.VA_DATAMART_HIST c горизонтом в 40 дней.
*
*  ПАРАМЕТРЫ:
*     Нет
*
...
*     %pc_daily_pg_va_dm_hist;
*
****************************************************************************
*  17-06-2020  Борзунов     Начальное кодирование
****************************************************************************/
 %macro pc_daily_pg_va_dm_hist; 
 	%local lmvCASSESS lmvReportDt lmvReportDttm lmvDateTreshold lmvInLibref ; 
	%let lmvCASSESS = casauto;
	%let lmvReportDt=&ETL_CURRENT_DT.;
	%let lmvReportDttm=&ETL_CURRENT_DTTM.;
	/*определение нижней границы (40 дней) */
	%let lmvDateTreshold = %sysfunc(intnx(day,&ETL_CURRENT_DT.,-40));
	...
	data CASUSER.PBO_SALES (replace=yes drop=channel_cd_old) ;
		set &lmvInLibref..PBO_SALES(where=(valid_from_dttm<=&lmvReportDttm and valid_to_dttm>=&lmvReportDttm
			and sales_dt>=%sysfunc(intnx(year,&lmvReportDt.,0)) and sales_dt<=&lmvReportDt.)
			rename=(channel_cd=channel_cd_old));
		length channel_cd $3;
		format channel_cd $3.;
	...
	data CASUSER.PMIX_SALES (replace=yes drop=channel_cd_old) ;
		set &lmvInLibref..PMIX_SALES(where=(valid_from_dttm<=&lmvReportDttm and valid_to_dttm>=&lmvReportDttm 
			and sales_dt>=%sysfunc(intnx(year,&lmvReportDt.,0)) and sales_dt<=&lmvReportDt.)
			rename=(channel_cd=channel_cd_old));
		length channel_cd $3;
		format channel_cd $3.;
	...
	data CASUSER.cost_price_full (replace=yes drop=start_dt end_dt) / SESSREF=&lmvCASSESS.;
		...
		do business_date=max(%sysfunc(intnx(year,&lmvReportDt.,0)),start_dt) to min(%sysfunc(intnx(year,&lmvReportDt.,0,e)),end_dt);
			if business_date >= &lmvDateTreshold. then output;
		end;
	run;
	...
	data CASUSER.PLAN_GC_HIST (replace=yes) / SESSREF=&lmvCASSESS.;
	    set CASUSER.PLAN_GC_MONTH (where=(mon_dt<=&ETL_CURRENT_DT.));
	...
	run;
	/* Отбор данных с горизонтом в 40 дней */
	data CASUSER.PLAN_UNIT_HIST (replace=yes) / SESSREF=&lmvCASSESS.;
	    set CASUSER.PLAN_UNITS_MONTH (where=(mon_dt<&ETL_CURRENT_DT. and mon_dt>=%sysfunc(intnx(year,&ETL_CURRENT_DT.,0))));
	    format business_date date9.;
	    mon_days_cnt=intnx('month',mon_dt,0,'e')-mon_dt;
	    do business_date=mon_dt to min(intnx('month',mon_dt,0,'e'),&ETL_CURRENT_DT.-1);
	...
	proc sql noprint;
		connect to postgres as DM_REP (&dm_rep_connect_options);
		execute by DM_REP (delete from dm_rep.va_datamart_hist 
							where business_date>=(current_date-40));
		disconnect from DM_REP;
	quit;

	proc append BASE=DM_REP.VA_DATAMART_HIST(bulkload=yes) DATA=CASUSER.VA_DATAMART_HIST (DATALIMIT=ALL)  FORCE;
	run;
	...
	proc casutil ;   
		...
		droptable incaslib="CASUSER" casdata="SALES_FULL_DATA" quiet;
