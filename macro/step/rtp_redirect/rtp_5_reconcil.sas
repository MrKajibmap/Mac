%macro rtp_5_reconcil(mpFSAbt = public.pbo_train,
	...
	%if %sysfunc(SESSFOUND(casauto)) = 0 %then %do; 
		...
		caslib _all_ assign;
	%end;
	%let mpScoreStartDt = &VF_FC_START_DT;
	%let mpScoreEndDt = &VF_FC_END_SHORT_DT;
	%local lmvLibrefRes lmvTabNmRes lmvLibrefAbt lmvTabNmAbt;
	...
	%let leadnhor=%eval(&VF_FC_END_SHORT_DT_SAS-&VF_FC_START_DT_SAS+1);
	proc cas;
