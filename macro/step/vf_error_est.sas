...
%macro vf_error_est;
	...
	proc cas;
		...
		timeId="SALES_DT"
		interval="month"
		casOut={caslib="public",name="pmix_fact_month",replace=True}
		;
		run;
	quit;
	...
	proc cas;
		...
		timeId="SALES_DT"
		interval="week.2"
		casOut={caslib="public",name="pmix_fact_week",replace=True}
		;
	run;
	quit;

	proc casutil;
		droptable casdata="TS_estimate_month" incaslib="public" quiet;
	run;
	...
	left join public.pmix_fact_month t2
		on t1.product_id=t2.product_id
		and t1.pbo_location_id=t2.pbo_location_id
	...
	proc casutil;
		promote casdata="TS_estimate_month" incaslib="casuser" outcaslib="public";
	run;
	...
	proc casutil;
		droptable casdata="TS_estimate_month_np" incaslib="public" quiet;
	run;
	...
	proc casutil;
		promote casdata="TS_estimate_month_np" incaslib="casuser" outcaslib="public";
	run;
	/*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*/
	proc casutil;
		droptable casdata="TS_estimate_week" incaslib="public" quiet;
	run;
	...
	proc fedsql sessref=casauto noprint;
		create table casuser.TS_estimate_week{options replace=true} as
			select t1.CHANNEL_CD
					...
					,t4.LVL2_ID
					,t4.LVL3_ID
			from &lmvWeekData t1 left join public.pmix_fact_week t2
				on t1.product_id=t2.product_id
				and t1.pbo_location_id=t2.pbo_location_id
				and t1.CHANNEL_CD=t2.CHANNEL_CD
	...
	proc casutil;
		promote casdata="TS_estimate_week" incaslib="casuser" outcaslib="public";
	run;
	/*-=-=-=-=-=-=-GC-=-=-=-=-=-=-=-=-=-=-*/
	proc cas;
		timeData.timeSeries result =r /
	...
	quit;
	proc casutil;
		droptable casdata="GC_estimate_month" incaslib="public" quiet;
	run;
	...
	proc casutil;
		promote casdata="GC_estimate_month" incaslib="casuser" outcaslib="public";
	run;
