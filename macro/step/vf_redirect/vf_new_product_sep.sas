...
proc fedsql sessref=casauto;
	create table public.npf_frame{options replace=true} as
		...
		from
			...
			(select distinct PROD_LVL2_ID from mn_long.product_dictionary) as t2	
	;
quit;
...
proc fedsql sessref=casauto;
	...
	create table public.npf_weekday_mean{options replace=true} as
		...
		inner join
			mn_long.product_dictionary as t2
		on
...
%text_encoding(mn_long.pbo_dictionary, A_AGREEMENT_TYPE);
%text_encoding(mn_long.pbo_dictionary, A_BREAKFAST);
%text_encoding(mn_long.pbo_dictionary, A_BUILDING_TYPE);
%text_encoding(mn_long.pbo_dictionary, A_COMPANY);
%text_encoding(mn_long.pbo_dictionary, A_DELIVERY);
%text_encoding(mn_long.pbo_dictionary, A_MCCAFE_TYPE);
%text_encoding(mn_long.pbo_dictionary, A_PRICE_LEVEL);
%text_encoding(mn_long.pbo_dictionary, A_DRIVE_THRU);
%text_encoding(mn_long.pbo_dictionary, A_WINDOW_TYPE);
...
%text_encoding(mn_long.product_dictionary, a_hero);
%text_encoding(mn_long.product_dictionary, a_item_size);
%text_encoding(mn_long.product_dictionary, a_offer_type);
%text_encoding(mn_long.product_dictionary, a_price_tier);
...
data public.macro (replace=yes  drop=valid_from_dttm valid_to_dttm);
...
run;
%if %sysfunc(exist(mn_long.media)) eq 0 %then %do;
	...
	data mn_long.media;
		...
		set mn_long.media_enh(rename=(report_dt=period_dt));	
	run;
%end;
